...
export function OnboardingImport({ onNext }: Props) {
  ...
  return (
    <div style={{ maxWidth: '34rem', width: '100%' }}>
      ...
      >
        ...
        <p style={{ margin: 0, fontWeight: 600, color: 'var(--color-on-surface)', fontSize: 'var(--font-body-large-size)' }}>
      ...
      <div style={{ display: 'flex', gap: '0.75rem' }}>
        <Button id="import-skip-btn" type="button" variant="secondary" onClick={() => onNext()}>
          Skip for now
        </Button>
        {!result ? (
          <Button id="import-btn" variant="primary" loading={loading} onClick={handleImport} disabled={!file} fullWidth>
            Import leads
          </Button>
        ) : (
          <Button id="import-next-btn" variant="primary" fullWidth onClick={() => onNext({ importedLeadIds: result.leadIds })}>
